/**
 * @module WxFetch
 * @description 测试 WxFetcg 模块
 */
import fetch from '../src/network/fetch.wx';
...
describe('wx-fetch', () => {
  it('fetch', async() => {
    ...
    // 测试请求
    const response = await fetch('https://m.api.test.solardc.com?_mt=octups.queryUser');
    ...
    // 测试请求
    const response2 = await fetch('https://m.api.test.solardc.com?_mt=octups.queryUser');
    // 断言:返回数据
  ...
  it('fetch.fail', async() => {
    ...
    try {
      // 测试请求
      await fetch('https://m.api.test.solardc.com?_mt=octups.queryUser');
    } catch (e) {
      error = e;
    }
    // 断言：这里需要触发异常
