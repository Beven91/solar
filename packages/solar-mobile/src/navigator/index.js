...
function StackNavigator(routeConfigs, stackConfig) {
  const { createNavigator, StackRouter } = Navigation;
  const routes = handleRouteConfig(routeConfigs);
  const navigator = createNavigator(StackRouter(routes, stackConfig))(NavigationViewer);
