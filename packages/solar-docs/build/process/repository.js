...
class GitRepository {
  ...
  clone(url, branch) {
    ...
    console.log('git clone ' + branch + ' ' + url);
    cp.spawnSync('git', ['clone', url, '--depth=1'], {
      cwd: reposRoot,
  ...
  async findRepository(repoMeta) {
    const dir = repoMeta.dir || path.join(reposRoot, path.basename(repoMeta.url).replace('.git', ''));
    ...
    for (let i = 0, k = repositories.length; i < k; i++) {
      ...
      sites.push({
        ...
        pages: files.map((element) => {
          return {
            ...element,
            name: element.page.replace(path.extname(element.page), ''),
            src: rootName + '/' + element.origin,
            filePath: path.join(element.dir, element.origin),
  ...
  findDocFiles(dir, repo, rootName) {
    ...
    return globby(patterns, { cwd: dir }).then((files) => {
      ...
      docFiles.forEach((name) => {
        const newName = name.split(path.sep).slice(-2).join(path.sep).replace(/\.\.\//g, '');
        const page = newName.replace(path.extname(newName), '.md');
    ...
    return globby(patterns, { cwd: dir }).then((files) => {
      files.forEach((file) => {
        if (!/node_modules/i.test(file)) {
          ...
          console.log('[solar-docs]: make type:' + file);
