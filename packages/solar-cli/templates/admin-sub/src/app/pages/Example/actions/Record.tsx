...
import { AbstractGroups } from 'solar-pc/src/interface';

export default class AbstractObjectTest extends React.Component {
  // 校验规则
  get rules() {
    return {
  ...
  get groups() {
    return [
      {
        ...
        items: [
          ...
          { title: '是否锁定', name: 'isLock', initial: true, render: <Switch /> },
        ],
      },
      {
        group: '配置信息',
    ...
    ] as AbstractGroups<ModelRecord>;
  }

  // 渲染
  render() {
    return <AbstractForm rules={this.rules} groups={this.groups} />;
  }
}
